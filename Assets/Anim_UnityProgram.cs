...
public partial class Anim_UnityProgram : MonoBehaviour
{
    //Core Components
    ...
    public void Start()
    {
        ...
        // 1. Load rig
        _unitySkeleton = gameObject.GetComponentInChildren<SkinnedMeshRenderer>();
        _rigRoot = _unitySkeleton.rootBone.gameObject;
        
        
        //AddChild(_rigRoot);

        // 2. Build skeleton
        _skeleton = UnityBridge.BuildSkeleton(_rigRoot);
        ...
        if (_unitySkeleton == null) Debug.LogError("[Bridge] No Skeleton3D found in rig hierarchy!");*/
       ...
       // ─────────────────────────────────────────────
        ...
        state_Dance.AddTransition("Locomotion", () => Input.GetKey(UnityEngine.KeyCode.R), 0.5f); // 0.5s "oh, gotta go" blend
        ...
        if (neckIndex != -1)
        {
            ...
            Debug.Log($"[IK] Added LookAt solver for bone 'mixamorig_Neck' (Index: {neckIndex})");
        }
        else
        {
            Debug.LogError($"[IK] Failed to find bone 'mixamorig_Neck' for LookAt solver.");
        }
    ...
    public void Update()
    {
        ...
        if (Input.GetKey(UnityEngine.KeyCode.Space))
        {
            if (!_spaceHeld)
            {
                _spaceHeld = true;
                _isPaused = !_isPaused;
                Debug.Log(_isPaused ? "[Runtime] Animation paused." : "[Runtime] Animation resumed.");
            }
        }
        ...
        {
            // 1. Input Pass
            HandleInput((float)Time.deltaTime);
            // 2. Update Graph
            ...
            // 6. Apply to Godot
            UnityBridge.ApplyBoneOverridesFromSkeleton(_unitySkeleton, _skeleton);
        }
    ...
    {
        // --- Locomotion ---
        bool isWalking = Input.GetKey(UnityEngine.KeyCode.R);
        bool isRunning = Input.GetKey(UnityEngine.KeyCode.LeftShift);
        float targetSpeed = 0.0f;
        if (isWalking)
        {
            targetSpeed = isRunning ? 1.0f : 0.5f;
        }
        _currentSpeed = Mathf.Lerp(_currentSpeed, targetSpeed, Time.deltaTime * 5.0f); 

        // --- Turning ---
        bool isTurningLeft = Input.GetKey(UnityEngine.KeyCode.LeftArrow);
        bool isTurningRight = Input.GetKey(UnityEngine.KeyCode.RightArrow);
        
        float targetTurn = 0.0f;
        if (isTurningLeft) targetTurn = -1.0f;
        else if (isTurningRight) targetTurn = 1.0f;
        _turnAmount = Mathf.Lerp(_turnAmount, targetTurn, Time.deltaTime * 7.0f);
        
        // --- State Triggers ---
        
        // Jump (V key) - This is a "one-shot" trigger
        if (Input.GetKey(UnityEngine.KeyCode.V))
        {
            if (!_jumpNode.IsPlaying && !_danceNode.IsPlaying) 
            {
                _triggerJump = true;
                Debug.Log("[Input] Jump Triggered!");
            }
        }
        
        if (Input.GetKey(UnityEngine.KeyCode.B)) 
        {
            if (!_jumpNode.IsPlaying && !_danceNode.IsPlaying) 
            {
                _isDancing = true;
                Debug.Log($"[Input] Dance Triggered!");
            }
        }
    }
    
    // ... UpdateAnimationGraph is unchanged ...
    private void UpdateAnimationGraph(float deltaTime)
    {
        // --- Update Locomotion ---
    ...
    private int[] BuildChain(string endBoneName, string rootBoneName)
    {
        ...
        {
            Debug.LogError($"[IK] Failed to build chain. Could not find '{endBoneName}' or '{rootBoneName}'.");
            return null;
        }
        ...
        for (int i = 0; i < _skeleton.BoneCount; i++)
        {
            if (current == null)
            {
                Debug.LogError($"[IK] Failed to build chain. Bone '{endBoneName}' is not a child of '{rootBoneName}'.");
                return null;
            }
        ...
        Debug.LogError($"[IK] Failed to build chain. Hit iteration limit.");
        return null;
    }
    ...
    private void RebuildLeftArmSolver(int selection)
    {
        ...
        if (chainIndices != null && chainIndices.Length > 0)
        {
            ...
            Debug.Log($"[IK] Built CCD solver for Left Arm (chain: {chainDesc})");
        }
        else
        {
            Debug.LogError($"[IK] Failed to build chain for selection {selection}.");
        }
    }
