...
namespace Nyteshade.Modules.Anim.Godot_Specific
{
    public static class GodotBridge
    {
        ...
        // ─────────────────────────────────────────────
        public static Godot.AnimationPlayer FindAnimationPlayerRecursive(Node node)
        {
            if (node is Godot.AnimationPlayer ap)
                return ap;
            ...
            foreach (var child in node.GetChildren())
            {
                if (child is Node n)
                {
                    var result = FindAnimationPlayerRecursive(n);
                    if (result != null)
                        return result;
                }
            }
        ...
        // Skeleton Builders
        // ─────────────────────────────────────────────
        public static Transform BuildTransformFromGodot(Node3D node)
        {
            if (node == null)
            ...
            var t = new Transform
            {
                ...
                Rotation = new Quaternion(node.Quaternion.X, node.Quaternion.Y, node.Quaternion.Z, node.Quaternion.W),
            ...
            foreach (var child in node.GetChildren())
            {
                ...
                {
                    var childTransform = BuildTransformFromGodot(child3D);
        ...
        public static Skeleton BuildSkeleton(Node3D root)
        {
            if (root == null)
