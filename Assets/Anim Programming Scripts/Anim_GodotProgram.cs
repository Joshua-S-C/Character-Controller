...
public partial class Anim_GodotProgram : Node
{
    //Core Components
    private Skeleton _skeleton;
    private Nyteshade.Modules.Anim.AnimationPlayer _player;
    private Node3D _rigRoot;
    ...
    private Dictionary<string, AnimationClip> _animLibrary = new Dictionary<string, AnimationClip>();
  ...
  public override void _Ready()
    {
        GD.Print("=== Nyteshade Animation Runtime Initialized ===");

        // ... (Steps 1-5 are identical to your file) ...
        // 1. Load rig
        var rigScene = GD.Load<PackedScene>("res://Assets/Skeleton_Rigged.glb");
        _rigRoot = rigScene.Instantiate<Node3D>();
        AddChild(_rigRoot);

        // 2. Build skeleton
        _skeleton = GodotBridge.BuildSkeleton(_rigRoot);
        ...
        GD.Print("[Init] Loading animation library...");
        ...
        GD.Print($"[Init] Loaded {_animLibrary.Count} animation clips.");
    ...
    private void DebugDrawBoneMotion()
    {
        if (_skeleton == null)
            return;

        ImGui.Begin("Bone Debugger");
        ImGui.Text($"Bone Count: {_skeleton.BoneCount}");
        ImGui.Text(_isPaused ? "Paused (Press Space)" : "Playing (Press Space)");
        
        ImGui.Separator();
        
        //Blend Tree Controls
        ImGui.Text("Locomotion (R, Shift)");
        ImGui.SliderFloat("Speed", ref _currentSpeed, 0.0f, 1.0f);
        ImGui.Text("Turning (Left/Right Arrows)");
        ImGui.SliderFloat("Turn Amount", ref _turnAmount, -1.0f, 1.0f);
        
        // [MODIFIED] Updated debug text
        ImGui.Text("--- STATES ---");
        ImGui.Text("Press [V] to Jump (One-Shot)");
        ImGui.Text("Press [B] to Dance (One-Shot)"); // Changed from "Toggle"
        ImGui.Text("Press [R] to interrupt Dance");
        
        // [MODIFIED] Updated state display logic
        ...
        ImGui.Separator();
        ImGui.Text("Look-At IK Controls (in Meters)");
        ImGui.SliderFloat("X (Left/Right)", ref _lookTargetPosGui.X, -2.0f, 2.0f);
        ImGui.SliderFloat("Y (Up/Down)", ref _lookTargetPosGui.Y, -2.0f, 0f);
        ImGui.SliderFloat("Look At Weight", ref _lookAtWeight, 0.0f, 1.0f);
        ImGui.Separator();
        ImGui.Text("Grab IK Controls (in Meters)");
        // ... (rest of ImGui is unchanged) ...
        ImGui.Text("Grab with:");
        if (ImGui.RadioButton("Hand (Full Arm)", ref _chainSelection, 0))
        {
            RebuildLeftArmSolver(0);
        }
        ImGui.SameLine();
        if (ImGui.RadioButton("Elbow (Forearm)", ref _chainSelection, 1))
        {
            RebuildLeftArmSolver(1);
        }
        ImGui.SameLine();
        if (ImGui.RadioButton("Shoulder (Upper Arm)", ref _chainSelection, 2))
        {
            RebuildLeftArmSolver(2);
        }
        ImGui.SameLine();
        if (ImGui.RadioButton("Wrist (Hand Only)", ref _chainSelection, 3))
        {
            RebuildLeftArmSolver(3);
        }

        ImGui.SliderFloat3("Grab Target", ref _grabTargetPosGui, -2.0f, 2.0f);
        ImGui.SliderFloat("Grab Weight", ref _grabWeight, 0.0f, 1.0f);
        ImGui.Separator();
        ImGui.Text("Bone Info (in Centimeters)");
        for (int i = 0; i < Math.Min(15, _skeleton.BoneCount); i++) 
        {
            var bone = _skeleton.GetBone(i);
            var p = bone.Position;
            ImGui.Text($"{i}: {bone.Name}");
            ImGui.TextColored(new System.Numerics.Vector4(0.3f, 1f, 0.3f, 1f),
